...
p.group2 <- "group 2"
p.cells <- 300
p.levels <- c("control", "comparison", ordered = TRUE)
...
for (i in 2:(strtoi(max.cluster)+1)){
  ...
  plt <- ggplot(data=count.table.condition,aes(x=factor(testvar, levels = c(p.group1, p.group2)), y=count.table.condition[,i])) +
    ...
    scale_color_manual(values = c("#000001", "#ed1202"))
  print(plt)
